...
async function loadPresets() {
  try {
    ...
    $('preset-list').innerHTML = state.presets.length ? state.presets.map(item => `<article class="preset-card job-card"><div class="job-card-heading"><div class="company-mark" aria-hidden="true">${esc((item.company || item.target_role).slice(0,1))}</div><div><h3>${esc(item.target_role)}</h3><p>${esc([item.company,item.city].filter(Boolean).join(' · ') || item.name)}</p></div></div><div class="job-card-meta"><span>${esc(item.education || '学历未填写')}</span><span>${esc(salaryLabel(item))}</span></div><p class="job-card-goal">${esc(item.coaching_goal || '从你的经历出发，练习这个岗位的真实面试')}</p><div class="preset-actions"><button class="btn secondary" type="button" data-preset-id="${escAttr(item.id)}" data-action="edit">编辑岗位</button><button class="btn primary" type="button" data-preset-id="${escAttr(item.id)}" data-action="start">选择岗位 · 配置面试</button></div></article>`).join('') : '<div class="jobs-empty"><strong>先创建一个目标岗位</strong><p>导入 JD、保存简历，之后选择岗位即可开始练习。</p></div>';
...
function openInterviewSetup(item) {
  if (!item) return;
  runJob = item;
...
async function startPresetInterview(event) {
  ...
  $('input-jd').value = runJob.jd; $('input-resume').value = runJob.resume;
  ...
  const ok = await startInterview(event, {coaching_goal:runJob.coaching_goal,interviewer_gender:selected('run-gender')});
